...
class RBM:
    ...
    def free_energy(self, sigma):
        ...
        hidden_term = torch.sum(torch.log(2 * torch.cosh(hidden_input)))
    ...
    def sample_hidden(self, sigma):
        ...
        tau = torch.where(torch.rand_like(prob) < prob, torch.ones_like(prob), -torch.ones_like(prob))
    ...
    def sample_visible(self, tau):
        ...
        sigma = torch.where(torch.rand_like(prob) < prob, torch.ones_like(prob), -torch.ones_like(prob))
    ...
    def data_average_dtheta_v(self, data_set):
        ...
        dtheta_v_array = torch.mean(data_set, dim=0)

        return dtheta_v_array
    ...
    def model_average_dtheta_v(self, data_set, k):
        ...
        dtheta_v_array = torch.mean(sigma, dim=0)
    ...
    def data_average_dtheta_h(self, data_set):
        ...
        dtheta_h_array = torch.mean(tau, dim=0)

        return dtheta_h_array
